...
def generate_audit_report(result, run_time, page_count):
    report = []
    report.append("======================================================================")
    report.append("   LOGIC MINER ENGINE - FINAL PRODUCTION AUDIT (PROTOCOL V.29)")
    report.append("======================================================================")
    report.append(f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"Corpus: Chemistry 2e (Full Volume)")
    report.append(f"Processing Depth: {page_count} Pages")
    report.append(f"Protocol: RIGOROUS ROOT & SCALING (V.29)")
    report.append(f"Status: UNIVERSAL TREE STABILIZED & SCALED (p={result['p']})")
    ...
    # Sort by Hierarchy (Coordinate)
    sorted_ents = sorted(result['entities'], key=lambda e: coords.get(e, 0))
    
    report.append(f"{'CONCEPT'.ljust(30)} | {'ADDR (p-adic)'.ljust(25)} | {'LOGIC TYPE'}")
    report.append("-" * 70)
    for ent in sorted_ents:
        p_val = coords.get(ent, 0)
        report.append(f"{ent.ljust(30)} | {str(p_val).ljust(25)} | {classific.get(ent, 'CONCEPT')}")

    report.append("\n" + "="*70)
    report.append("   V.29 AUDIT COMPLETE - THE ONTOLOGY IS RESOLVED & CLEAN")
    report.append("="*70)
    
    return "\n".join(report)

def main():
    print("--- [Logic Miner V.29 - Rigorous Root & Scaling Run] ---")
    miner = LogicMiner()
    ...
    print(f"   > Initiating V.29 Rigorous Synthesis on {total_pages} pages...")
