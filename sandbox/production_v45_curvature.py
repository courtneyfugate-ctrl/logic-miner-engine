...
from logic_miner.core.serial_synthesis_v45 import SerialSynthesizerV45
from pypdf import PdfReader

def generate_audit_report(result, run_time, page_count):
    report = []
    report.append("======================================================================")
    report.append("   LOGIC MINER ENGINE - CURVATURE AUDIT (V.45)")
    report.append("======================================================================")
    report.append(f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"Corpus: Chemistry 2e (Full Volume)")
    report.append(f"Protocol: THE CURVATURE MODEL (V.45)")
    report.append(f"Components: V.32 Base + V.45 Discrete Curvature Filter")
    report.append(f"Status: STABILIZED (p={result['p']})")
    
    report.append("\n" + "-"*80)
    report.append("ADELIC FOREST TRACE: CURVATURE-FILTERED LATTICE")
    ...
    report.append("\n" + "="*80)
    if purity_failure:
        report.append("   [WARNING] Purity Breach (Flat Terms Detected).")
    else:
        report.append("   [SUCCESS] Curvature Purity Verified.")
    report.append("="*80)
    
    return "\n".join(report)

def main():
    print("--- [Logic Miner V.45 - The Curvature Model] ---")
    miner = LogicMiner()
    ...
    print(f"   > Initiating V.45 Curvature Synthesis (K > 0.5)...")
    
    # V.45 Synthesizer
    synthesizer = SerialSynthesizerV45(chunk_size=50)
    ...
    dump_path = "sandbox/v45_curvature_audit.txt"
