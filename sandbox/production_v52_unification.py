...
from logic_miner.core.serial_synthesis_v52 import SerialSynthesizerV52
from pypdf import PdfReader

def generate_tree_report(synthesizer, run_time, page_count):
    report = []
    report.append("======================================================================")
    report.append("   LOGIC MINER ENGINE - GRAND UNIFICATION (V.52)")
    report.append("======================================================================")
    report.append(f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"Corpus: Chemistry 2e (Full Volume)")
    report.append(f"Protocol: THE GRAND UNIFICATION")
    report.append(f"Components: Splines + Independent Primes + Frequency Gravity")
    report.append(f"Basis: p = [5, 7, 11]")
    report.append(f"Status: RESTORED")
    
    report.append("\n" + "-"*80)
    report.append("ADELIC DENDROGRAM: V.52 UNIFIED TREE")
    ...
    def print_node(node, depth=0, visited=None):
        ...
        # Sort children by frequency (Gravity)
    ...
    for root in roots:
        ...
        report.append("") # Spacer between major trees

    report.append("\n" + "="*80)
    report.append(f"   [STATS] Total Terms: {total_yield}")
    report.append(f"   [STATS] Total Roots (Trunks): {total_roots}")
    
    # Verification Logic
    if total_roots <= 10:
        report.append("   [SUCCESS] Unification Verified (<= 10 Major Trunks).")
    else:
        report.append(f"   [WARNING] Still fragmented ({total_roots} trunks). Check Gravity.")
...
def main():
    print("--- [Logic Miner V.52 - The Grand Unification] ---")
    ...
    print(f"   > Initiating V.52 Synthesis (Splines + Primes + Gravity)...")
    
    # V.52 Synthesizer
    synthesizer = SerialSynthesizerV52(chunk_size=50, momentum=0.3)
    synthesizer.fit_stream(reader=reader, max_pages=pages_to_run)
    
    # Call Consolidate Explicitly (The Tree Builder)
    ...
    dump_path = "sandbox/v52_unification_audit.txt"
    with open(dump_path, "w", encoding='utf-8') as f:
        f.write(report_content)
        
    print(f"\n   > Production Run Complete. Result saved to: {dump_path}")
    print(f"   > Time Elapsed: {run_time:.2f}s")
    print(f"   > Total Yield: {len(synthesizer.final_vectors)} terms")
